  ...
  return(
    <div style={{padding:"64px 5%",maxWidth:900,margin:"0 auto"}}>
      <div style={{marginBottom:40}}>
        <div style={{fontSize:"0.72rem",fontWeight:600,letterSpacing:"0.18em",textTransform:"uppercase",color:"#c06b3a",marginBottom:12,display:"flex",alignItems:"center",gap:10}}><span style={{display:"block",width:20,height:1.5,background:"#c06b3a"}}/>Get In Touch</div>
        ...
        <p style={{color:"#8a7f72",fontSize:"0.95rem",lineHeight:1.7}}>Have a question about booking or listing? Send us a message and we'll get back to you within 24 hours.</p>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40}}>
        <div>
          {[["name","Your Name *","text","Your name"],["email","Email Address *","email","you@email.com"]].map(([k,label,type,ph])=>(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20}}>
            ...
            <select style={{border:"1.5px solid #d9c9a8",borderRadius:3,padding:"12px 16px",fontSize:"0.92rem",background:"#fff",color:"#3b2e22",outline:"none",appearance:"none"}} value={form.subject} onChange={e=>upd("subject",e.target.value)}>
              <option value="">Select a topic</option>
              <option>Booking Question</option><option>Listing My Space</option><option>Payment Issue</option><option>Cancellation Request</option><option>Report a Problem</option><option>General Question</option>
            </select>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20}}>
          ...
          <button style={{background:"#c06b3a",color:"#fff",border:"none",borderRadius:3,padding:"16px 40px",fontSize:"0.88rem",fontWeight:500,letterSpacing:"0.06em",textTransform:"uppercase",cursor:"pointer",width:"100%",opacity:loading||!form.name||!form.email||!form.message?0.5:1}} onClick={submit} disabled={loading||!form.name||!form.email||!form.message}>{loading?"Sending…":"Send Message →"}</button>
        </div>
        <div>
          <div style={{background:"#f5f0e8",borderRadius:6,padding:28,marginBottom:20}}>
            ...
            {[["📧","Email","hello@yardevents.net"],["🌐","Website","yardevents.net"],["⏰","Response Time","Within 24 hours"],["📍","Based in","Perrysburg, Ohio"]].map(([icon,label,val])=>(
              <div key={label} style={{display:"flex",gap:12,marginBottom:16}}>
                ...
                <div><div style={{fontSize:"0.72rem",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"#c06b3a",marginBottom:2}}>{label}</div><div style={{fontSize:"0.9rem",color:"#3b2e22"}}>{val}</div></div>
              </div>
            ))}
          </div>
          <div style={{background:"#3b2e22",borderRadius:6,padding:28}}>
            <h3 style={{fontFamily:"Cormorant Garamond,serif",fontSize:"1.2rem",marginBottom:12,color:"#c9a84c"}}>Are you a Host?</h3>
            <p style={{fontSize:"0.88rem",lineHeight:1.7,color:"rgba(245,240,232,0.75)",marginBottom:16}}>List your backyard, barn, or private land. Set your own hourly or daily rate and start earning!</p>
...
export function HelpPage(){
  ...
  const faqs=[
    {category:"For Guests",items:[
      {q:"How do I book a space?",a:"Browse spaces, click on one you like, select your date, choose your start time and how many hours (for hourly spaces), then click Reserve This Space."},
      {q:"Can I book for just a few hours?",a:"Yes! Many hosts offer hourly rates. You can book as little as the host's minimum (usually 1–2 hours) up to however many hours you need. The price is calculated automatically."},
      {q:"How much does it cost?",a:"Each host sets their own hourly or daily rate. You also pay a 12% service fee. The total is always shown before you pay — no surprises."},
    ...
    {category:"For Hosts",items:[
      {q:"Can I set an hourly rate?",a:"Yes! When listing your space you choose between hourly or daily pricing. With hourly pricing you also set a minimum booking duration (e.g. 2 hours minimum)."},
      {q:"Can I set a minimum number of hours?",a:"Absolutely. You can set a minimum of 1, 2, 3, 4, 5, or 6 hours. Guests cannot book fewer hours than your minimum."},
      {q:"Is there a maximum booking duration?",a:"No maximum — guests can book as many hours as they need. You set the minimum, they choose the duration. Great for flexibility!"},
      {q:"How do I get paid?",a:"You keep 88% of every booking. Payouts are sent to your bank account within 24 hours of each completed event via Stripe."},
    ...
    {category:"Payments & Safety",items:[
      {q:"Is my payment safe?",a:"Yes! All payments are processed by Stripe, one of the world's most trusted payment platforms. Your card info is encrypted and never stored on our servers."},
      {q:"What payment methods do you accept?",a:"All major credit and debit cards — Visa, Mastercard, American Express, and Discover."},
      ...
      {faqs.map((section,si)=>(
        <div key={si} style={{marginBottom:40}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {section.items.map((faq,fi)=>{const id=`${si}-${fi}`;return(
    ...
    <footer style={{background:"#3b2e22",padding:"48px 5% 28px",marginTop:60}}>
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:40,marginBottom:40}}>
        <div>
          ...
          <p style={{fontSize:"0.85rem",lineHeight:1.7,color:"rgba(245,240,232,0.5)",maxWidth:260}}>The marketplace for private event spaces. Book by the hour or the day. Connecting hosts with guests for unforgettable celebrations.</p>
        </div>
        <div>
          ...
          {[["Browse Spaces","browse"],["List Your Space","host-dash"],["Help Center","help"],["Contact Us","contact"]].map(([label,page])=>(
            <div key={label} style={{marginBottom:10}}><button onClick={()=>onNavigate(page)} style={{background:"none",border:"none",color:"rgba(245,240,232,0.5)",fontSize:"0.85rem",cursor:"pointer",padding:0,textAlign:"left"}}>{label}</button></div>
          ))}
        </div>
        <div>
